...
import { FiMenu, FiX } from 'react-icons/fi';  // Usamos react-icons
...
export function Navbar() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-black/80 backdrop-blur-md shadow-md' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          >
            {isMobileMenuOpen ? <FiX size={24} /> : <FiMenu size={24} />} {/* Aquí los íconos de react-icons */}
          </button>
        </div>
      </div>
